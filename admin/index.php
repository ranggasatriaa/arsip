...
<body>
  <div class="page">
    ...
    <div class="page-content d-flex align-items-stretch">
      ...
      <div class="content-inner">
                            ...
                            if ((int)$mulai > -1) {
                              ...
                              while ($datauser = $listuser->fetch_object()) {
                                ...
                                <tr>
                                  ...
                                  <td>
                                    <?php if ((int)$datauser->user_level == 0) { ?>
                                      <button type="button" data-toggle="modal" data-target="#modalUpgrade" class="btn btn-primary middle btn-sm" data-name="<?=$datauser->user_name?>" data-email="<?=$datauser->user_email?>" data-key="<?=$datauser->user_key?>">Upgrade</button>
                                    <?php } else { ?>
                                      <button type="button" data-toggle="modal" data-target="#modalDowngrade" class="btn btn-secondary middle btn-sm" data-name="<?=$datauser->user_name?>" data-email="<?=$datauser->user_email?>" data-key="<?=$datauser->user_key?>">Downgrade</button>
        ...
        <div id="modalUpgrade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
          <div role="document" class="modal-dialog">
            <div class="modal-content">
              <form id="formUpgrade" method="get" class="table">
                <div class="modal-header">
                  <h4 id="exampleModalLabel" class="modal-title">User Upgrade</h4>
                ...
                <div class="modal-body">
                  <div class="table-responsive">
                    <table class="table">
                      <tr>
                        <td>Nama user</td>
                        <td id="text-user-name-upgrade"></td>
                      </tr>
                      <tr>
                        <td>Email</td>
                        <td id="text-user-email-upgrade"></td>
                      </tr>
                    </table>
                  </div>
                  <input type="hidden" name="user-key" class="form-control" id="user-key-upgrade">
                </div>
        ...
        <div id="modalDowngrade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
          <div role="document" class="modal-dialog">
            <div class="modal-content">
              <form id="formDowngrade" method="get" class="table">
                <div class="modal-header">
                  <h4 id="exampleModalLabel" class="modal-title">User Downgrade</h4>
                  <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                </div>
                <div class="modal-body">
                  <div class="table-responsive">
                    <table class="table">
                      <tr>
                        <td>Nama user</td>
                        <td id="text-user-name-downgrade"></td>
                      </tr>
                      <tr>
                        <td>Email</td>
                        <td id="text-user-email-downgrade"></td>
                      </tr>
                    </table>
                  </div>
                  <input type="hidden" name="user-key" class="form-control" id="user-key-downgrade">
                </div>
                <div class="modal-footer">
                  <button type="button" data-dismiss="modal" class="btn btn-secondary">Close</button>
  ...
  <?php 
  echo '<script>var status = '.(isset($_SESSION['status']) ? $_SESSION['status'] : 0).'; var message = "'.(isset($_SESSION['message']) ? $_SESSION['message'] : '').'";</script>'; 
  ...
  $('#modalUpgrade').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    modal.find('#formUpgrade').attr('action','upgrade.php?user-key='+button.data('key'));
    modal.find('.modal-body #text-user-name-upgrade').text(button.data('name'))
    modal.find('.modal-body #text-user-email-upgrade').text(button.data('email'))
    modal.find('.modal-body #user-key-upgrade').val(button.data('key'))
  })

  $('#modalDowngrade').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget)
    var modal = $(this)
    modal.find('#formDowngrade').attr('action','downgrade.php?user-key='+button.data('key'));
    modal.find('.modal-body #text-user-name-downgrade').text(button.data('name'))
    modal.find('.modal-body #text-user-email-downgrade').text(button.data('email'))
    modal.find('.modal-body #user-key-downgrade').val(button.data('key'))
  })
